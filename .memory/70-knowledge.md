...
Workforce (`WorkForceManagement`): **not** Manage FILTER / `AcGetData TEBWorkforce`. Live screens: `/workforce/team`, `/workforce/route`, `/workforce/manage` (Day), `/workforce/insights` (Attendance/Month). **Find my team** — `GetUserDropdown?Module=TEBWorkforce` (then team-member / tree fallbacks) → POST `GetSignedInUsersLastLocation` with the UserId list → last-known pins. Last-location `Address` is often null/`0`; fill `CurrentAddress` / `StartAddress` from POST `CurrentDateUserTrackingInfo` `{ UserId, CurrentDate }`. A named user (`Where is Priya`, `Where is the user now`) — `GetUserLastLocation` plus the same tracking address merge. Never show a 24-char UserId as the name (dropdown `Text` / `FullName` / `UserName`). **Route** (`Show route for me`, `Route of {username}`) — POST `GetUserTrackingMapView` with `{ UserId, CurrentDate, DisplayType: STARTDAY, Accuracy: Default, Distance: Default }`, then start/end + `PolylineList` (`Path`). `GetUserTrackingListView` has the start/end street address (`StartCoordinate.Address`, `ListRecords[].Address`) even when MapView `Address` is null. Maps render as OSM pins + path (`ReportResult.map` / `paths`), never lat/long bar charts.

**People started / members present {date}** is punch-in, not HR joining. Management Dashboard Workforce card: POST `gateway/reporting/GetOverviewWorkforceSnapshot` (`ChartCode: WORKFORCESNAPSHOT`, `ModuleCode: TEBWorkforce`) with reporting `DateFilter` calendar `YYYY-MM-DD` (not ISO-sliced UTC). Stages: `STARTDAY` (Start Day), `ENDDAY` (End Day), `FORCEDENDDAY` (Force End Day), `NOTSTARTDAY` (Not Started). Day Manage names: POST `gateway/workforce/GetTrackingSummary` (last 90 days) or `GetArchiveTrackingSummary` (older) with `{ Data: { StartEventTime, EndEventTime, UserIds, EventTypes: [] }, PageNumber, PageSize }`. `UserIds` are Workforce dropdown **`Id`** values; an empty list 500s / is skipped in live UI. A row with `StartDate` has punched in; `EndDate` has punched out. Attendance/Month is `gateway/Workforce/GetSubscriberUserAttendance` (`MemberIds` required, `TeamIds` allowed). Do not use `DateOfJoining` for this question. Phrases: `How many people started today`, `no of people have started yesterday`, `Members present today`. `joined on` / `joining date` stay HR joining.
