...
- Ask chatbot: NL → GetFilterControls + lookups → FilterValues / reporting FilterDetail → list or count report. Company/Contact use `GetCustomerDetail` + `getcustomerdetails` + `GETCOMPANYDETAIL` / `GETCONTACTDETAIL` (phone/email click-to-call cards). Quote view in Ask is read-only (`GETQUOTEDETAIL`, `VIEWCONSUMEDITEM`, templates, notes, activity) plus an Open-in-TEB link to live.teb.cloud. Template PDFs preview in chat via TEMPLATE `AcDownloadPdf`. Items and totals show as a receipt card (empty trailing “Item” / 0 placeholder rows are omitted). **Leads created by {user}** uses the live OWNER dropdown and `CreatedBy` / `OwnerName`. Last-N-days includes today (local calendar BETWEEN). Analysis sums `QuoteCurrentValue` / `QuoteNetValue` and bolds counts/amounts. No new backend.
